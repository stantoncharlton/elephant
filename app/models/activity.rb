class Activity < ActiveRecord::Base
    ...
    def self.activities_for_jobs(jobs)
        ...
        if Rails.env == "development"
            ...
            select("DISTINCT ON (activities.job_id) *").where("activities.job_id IN (?)", jobs.map { |j| j.id }).where("activity_type >= :start_range AND activity_type <= :end_range", start_range: 100, end_range: 200).order("activities.created_at DESC, activities.job_id DESC")
        end
    end
    ...
    def self.activities_for_user_today(user)
        ...
        if Rails.env == "development"
            ...
            select("DISTINCT ON (activities.job_id) *").where("activities.job_id IN (?)", user.jobs.map { |j| j.id }).where("created_at > ?", (Time.now - 1.day)).where("activity_type >= :start_range AND activity_type <= :end_range", start_range: 100, end_range: 200).order("activities.created_at DESC, activities.job_id DESC")
        end
