module ExploreHelper
    ...
    def get_all_assets_rops time_range, group
        ...
        result = DrillingLog.joins(drilling_log_entries: { bha: { bha_items: :tool } }).where("drilling_logs.updated_at >= ?", Date.today - 3.months).select("distinct on(asset_name) part_memberships.name as asset_name, avg(coalesce(drilling_logs.drilling_rop, 0)) AS rop, sum(drilling_logs.npt) as npt, sum(drilling_logs.total_time) as total_time").group("asset_name")
        if group == 1
