        ...
        if @district.present?
            if current_user.role.district_read? && !@district_present
                ...
                if current_user.role.limit_to_assigned_jobs?
                    @average_redress = PartRedress.includes(:job).where("part_redresses.company_id = ?", current_user.company_id).where("jobs.id in (select job_id from job_memberships where user_id = :user_id)", user_id: current_user.id).average("part_redresses.finished_redress_at - part_redresses.received_at").to_f.round(1)
