class ClientsController < ApplicationController
    ...
    before_filter :signed_in_user_not_field, only: [:edit, :update, :destroy]
    set_tab :clients
    ...
    def show
        ...
        if params[:search] && !params[:search].blank?
            ...
            @jobs = @client_jobs.includes(dynamic_fields: :dynamic_field_template).includes(:field, :well, :documents, :district, :client, :job_template => {:primary_tools => :tool}).includes(job_template: {product_line: {segment: :division}}).paginate(page: params[:page], limit: 20)
        end
    end
