class DivisionsController < ApplicationController
    ...
    def show
        ...
        if @is_paged
            ...
            @jobs = Job.includes(dynamic_fields: :dynamic_field_template).includes(:field, :well, :job_processes, :documents, :district, :client, :job_template => {:primary_tools => :tool}).includes(job_template: {product_line: {segment: :division}}).where("jobs.company_id = :company_id AND divisions.id = :division_id", company_id: @division.company_id, division_id: @division.id).where("jobs.status = :status_active OR (jobs.status = :status_closed AND jobs.close_date >= :close_date)", status_active: Job::ACTIVE, status_closed: Job::CLOSED, close_date: (Time.now - 5.days)).
