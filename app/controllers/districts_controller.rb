class DistrictsController < ApplicationController
    ...
    def show
        ...
        if params[:search] && !params[:search].blank?
            ...
            @jobs = @district_jobs.includes(dynamic_fields: :dynamic_field_template).includes(:field, :well, :documents, :district, :client, :job_template => {:primary_tools => :tool}).includes(job_template: {product_line: {segment: :division}}).paginate(page: params[:page], limit: 20)
        end
    end
